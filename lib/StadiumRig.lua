...
--
-- `waterBody`, when true, tells the shared shader this rig is allowed to
-- have real submerged geometry of its own (a water-type Pokemon modeled
-- with part of its body below its own y=0) -- see the `waterBody` uniform
-- note in Voxel3D's SHADER. Leave it nil/false for anything standing on
-- dry land: a rig's foot dipping slightly below zero mid-stride is not
-- water and should not get the wave/tint treatment.
--
...
function StadiumRig:draw(matrix, pull, waterBody)
  Voxel3D.seams(false)
  Voxel3D.glass(false)
  local additive = nil
  for _, part in ipairs(self.parts) do
