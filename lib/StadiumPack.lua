...
local function readPack(species, shiny)
  ...
  -- Also check Stadium 2 cache directory for DSM4 files
  -- Stadium 2 uses different naming: NNN_shiny.dsm instead of NNNs.dsm
  local rel2
  if shiny then
    rel2 = ("dramatic_shape/%03d_shiny.dsm"):format(species)
  else
    rel2 = ("dramatic_shape/%03d.dsm"):format(species)
  end
  if love and love.filesystem and love.filesystem.getInfo then
    local okInfo, info = pcall(love.filesystem.getInfo, rel2, "file")
    if okInfo and info then
      local ok, bytes = pcall(love.filesystem.read, rel2)
      if ok and type(bytes) == "string" and #bytes > 4 then return bytes end
    end
  end

  -- Check Stadium 2 shipped assets
  if mod and mod.read then
    local shippedPath
    if shiny then
      shippedPath = ("assets/stadium2/%03d_shiny.dsm"):format(species)
    else
      shippedPath = ("assets/stadium2/%03d.dsm"):format(species)
    end
    local ok, bytes = pcall(mod.read, mod, shippedPath)
    if ok and type(bytes) == "string" and #bytes > 4 then return bytes end
  end
...
-- ------- DSM4 unpacker (Stadium 2 format)
--
-- Stadium 2 uses a simpler format with table-based bone structure
-- and stub animations. This is a minimal unpacker that converts
-- DSM4 to the same model structure as DSM3 for compatibility.

local function unpackDSM4(bytes)
  ...
  -- Read bones (table-based structure in DSM4)
  local bones = {}
  for i = 1, nBones do
    local parent = i16()
    local t = { i16(), i16(), i16() }
  ...
  -- Read textures (just dimensions, no data in DSM4)
  local textures = {}
  for i = 1, nTextures do
    ...
    textures[i] = {
      w = w,
      h = h,
      rgba = ""  -- Empty for DSM4, textures loaded separately
    }
  end

  -- Convert DSM4 model structure to DSM3-compatible format
  local model = {
    bytes = bytes,
    species = species,
    boneCount = nBones,
    ...
    moveAux = moveAux,
    ctx = contexts,
    -- Convert table-based bones to flat arrays for compatibility
    parent = {},
    restT = {},
    restR = {},
    restS = {},
    ...
    anims = {},
    auxAnims = {},
    isDSM4 = true  -- Flag to identify DSM4 models
  }

  -- Flatten bone structure
  for i = 1, nBones do
    model.parent[i] = bones[i].parent + 1  -- Convert to 1-based
    local b = (i - 1) * 3
    model.restT[b + 1] = bones[i].t[1]
    model.restT[b + 2] = bones[i].t[2]
    model.restT[b + 3] = bones[i].t[3]
    model.restR[b + 1] = bones[i].r[1]
    model.restR[b + 2] = bones[i].r[2]
    model.restR[b + 3] = bones[i].r[3]
    model.restS[b + 1] = bones[i].s[1]
    model.restS[b + 2] = bones[i].s[2]
    model.restS[b + 3] = bones[i].s[3]
  end

  -- Create stub animations
  for i = 1, nAnims do
    model.anims[i] = {
      name = "anim_" .. i,
      frames = 1,
      loopStart = 0,
      aux = nil,
      seconds = 1 / StadiumPack.FPS,
      offset = 0,
      tracks = nil  -- DSM4 has no track data
    }
  end
...
-- The model for a National Dex number (1..251), or nil.
--
...
function StadiumPack.load(species, shiny)
  ...
  local ok, model = pcall(function()
    local magic = bytes:sub(1, 4)
    if magic == "DSM4" then
      -- Use DSM4 unpacker for Stadium 2
      local m, err = unpackDSM4(bytes)
      if not m then
    ...
    elseif magic == "DSM3" then
      ...
      if not readTextures then error("readTextures is nil", 0) end
      p = readTextures(bytes, p, m)
      if not readAnims then error("readAnims is nil", 0) end
      p = readAnims(bytes, p, m)
      if not readAux then error("readAux is nil", 0) end
      readAux(bytes, p, m)
      return m
    else
