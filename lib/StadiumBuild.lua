...
function StadiumBuild.species(rom, fileno)
  ...
  local ok, err = pcall(function()
    -- Detect if this is a Stadium 2 ROM by checking if it has StadiumRom2 methods
    local isStadium2 = rom.modelCount and rom.modelCount(rom) > 151
    if isStadium2 then
      -- Stadium 2 uses rare color metadata from ROM
      local rare = rom:rareColour(species)
      if rare and not rare.specialTexture then
        -- Apply rare color transformation to textures
        for i, tex in ipairs(data.textures) do
          if tex.rgba and #tex.rgba > 0 then
