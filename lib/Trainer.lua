...
local WINDOWS_RUNTIME=PLATFORM_OS=="Windows"

-- Mesh layouts come from TrainerMorph so Trainer and PlayerTrainer cannot drift
-- apart. v27 DENSE preserves source HSD RGBA beside XYZ/UV/normal; COMPACT is
-- only used by the CPU fallback binding path.
...
uniform float alphaComp0; uniform float alphaRef0; uniform float alphaOp;
uniform float alphaComp1; uniform float alphaRef1;
varying vec3 worldPos;
varying vec3 worldNormal;
varying vec4 sourceColor;
bool sourceAlphaCompare(float value,float mode,float refValue) {
  float a=floor(clamp(value,0.0,1.0)*255.0+0.5);
  float r=floor(clamp(refValue,0.0,255.0)+0.5);
  if (mode<0.5) return false;
  if (mode<1.5) return a<r;
  if (mode<2.5) return a==r;
  if (mode<3.5) return a<=r;
  if (mode<4.5) return a>r;
  if (mode<5.5) return a!=r;
  if (mode<6.5) return a>=r;
  return true;
}
bool sourceAlphaPass(float value) {
  bool a=sourceAlphaCompare(value,alphaComp0,alphaRef0);
  bool b=sourceAlphaCompare(value,alphaComp1,alphaRef1);
  if (alphaOp<0.5) return a && b;
  if (alphaOp<1.5) return a || b;
  if (alphaOp<2.5) return a != b;
  return a == b;
}
vec4 effect(vec4 color, Image texture, vec2 uv, vec2 screen) {
  ...
  /* HSD material diffuse still modulates textured DOBJ groups.  The old path
     replaced it with raw texture RGB whenever a texture existed, washing out
     source-tinted clothes/skin/material accents. */
...
local function writeTrainerRuntimeMeta(wanted,cache,sourceSize,preserveExisting)
  ...
  local ok=RuntimeMeshCache.writeLua(trainerRuntimeMetaPath(wanted),out,preserveExisting)
...
local function imageFromRaw(spec,wrapS,wrapT)
  ...
  if img.setFilter then
    ...
    local magName=(tonumber(spec.magFilt) or 1)==0 and "nearest" or "linear"
    local gxAniso=math.max(0,math.min(2,math.floor(tonumber(spec.maxAnisotropy) or 0)))
    local anisotropy=2^gxAniso
    local okf=pcall(img.setFilter,img,minName,magName,anisotropy)
    if not okf then pcall(img.setFilter,img,minName,magName) end
  end
...
local function trainerModelFor(ctx)
  ...
  if b and b.__mtbHub==true then return nil,"mtbattle-hub-player-only" end
...
local function loadConfig(ctx,cfg,reason)
  ...
  local rt
  local preserveTrainerRuntime=false
  if DENSE_MESH and RuntimeMeshCache and type(RuntimeMeshCache.readLua)=="function" then
    local rtPath=trainerRuntimeMetaPath(wanted)
    rt=select(1,RuntimeMeshCache.readLua(rtPath))
    preserveTrainerRuntime=rt~=nil or (type(RuntimeMeshCache.exists)=="function" and RuntimeMeshCache.exists(rtPath)) or false
  ...
  for i,g in ipairs(cache.groups or {}) do
    ...
    if fromRuntime and RuntimeMeshCache and type(RuntimeMeshCache.meshFromPath)=="function" then
      mesh,meshErr=RuntimeMeshCache.meshFromPath(FORMAT,binPath,48,"static")
    end
    if not mesh then
      ...
      if fromRuntime and type(g.vertices)~="table" then
        if canonicalFallback==nil then
          ...
          if type(canonical)=="table" and tonumber(canonical.formatVersion)==27 then canonicalFallback=canonical else canonicalFallback=false;meshErr=tostring(cerr or meshErr or "canonical trainer fallback unavailable") end
        end
      ...
      for _,row in ipairs(vertices) do if type(row)=="table" then row[49]=nil end end
      ...
      if DENSE_MESH and RuntimeMeshCache and RuntimeMeshCache.supported and RuntimeMeshCache.supported() then
        local wok=RuntimeMeshCache.writeRows(binPath,vertices,48,nil,preserveTrainerRuntime)
  ...
  if not fromRuntime and RuntimeMeshCache and RuntimeMeshCache.supported and RuntimeMeshCache.supported() and sourceSize then
    ...
    for i=1,#(cache.groups or {}) do
      ...
      local size=tonumber(info.size)
      if size and (size<192 or size%192~=0) then all=false;break end
    end
    if all then writeTrainerRuntimeMeta(wanted,cache,sourceSize,preserveTrainerRuntime) end
  end
...
function T:begin(ctx)
  ...
  age=0;actionKind=nil;actionAge=0;actionStrength=0;initialSendoutQueued=false;initialOpeningQueued=false;lastSendingOut=false;pendingFrustration=nil;pendingApproval=nil;pendingReaction=nil;resultSeen=nil;drawFrames=0;currentMotion=nil
...
function T:update(ctx,dt)
  local b=battleOf(ctx)
  if b~=battleKey then
    battleKey=b;age=0;actionKind=nil;actionAge=0;actionStrength=0;initialSendoutQueued=false;initialOpeningQueued=false;lastSendingOut=false;pendingFrustration=nil;pendingApproval=nil;pendingReaction=nil;resultSeen=nil;activeNow=false;activationReason=nil;currentMotion=nil
  ...
  elseif not should and activeNow then
    activeNow=false;age=0;actionKind=nil;actionAge=0;actionStrength=0;initialOpeningQueued=false;pendingFrustration=nil;pendingApproval=nil;pendingReaction=nil;resultSeen=nil;activationReason=nil;currentMotion=nil
  end
  ...
  elseif activeNow and pendingApproval then
    pendingApproval=pendingApproval-step
...
function T:event(ctx,name,payload)
  ...
  elseif name=="battle.battler_switched" then
    -- GC6E01's ordinary return path (fightOutPokemonModosuEffect) has no
    -- companion fightTrainer animation. The Pokemon/WZX return owns this beat;
    -- leave the trainer's current/idle track alone and only arm the next real
    -- enemySendingOut edge. This also prevents a guessed recall clip from
    -- holding equal priority with the following retail ball-throw action.
  ...
  elseif semantic=="battle.fainted" then
    ...
    elseif fainted=="player" and not pendingFrustration then
      local fd=BattleDirector and type(BattleDirector.faintDuration)=="function"
...
local function animatedModel(p,motion)
  -- Full chronological source tracks already include the trainer's authored
  -- root/body motion.  Applying the semantic performance root on top of those
  -- vertices doubled lean, sway and translation and made otherwise-correct
  -- Colosseum clips look exaggerated.  Procedural root motion remains only for
  -- old/unsupported caches that genuinely need the fallback.
  ...
  return Mat4.mul(Mat4.translate(p.x+sway,p.y+bob-(root.compression or 0),p.z+forward),
...
local function setShader(vp,model,pose,unlit,tint,opacity,motion)
  ...
  shader:send("sourceVertexColor",0)
  shader:send("sourceVertexAlpha",0)
  shader:send("alphaComp0",7);shader:send("alphaRef0",0);shader:send("alphaOp",0)
  shader:send("alphaComp1",7);shader:send("alphaRef1",0)
...
function T:drawShadow(ctx,vp,pose)
  ...
  local sourceRoot=s.nativeTrack and TrainerMorph.nativeTrackOwnsRoot
    and TrainerMorph.nativeTrackOwnsRoot(s.nativeTrack,motion)
  local model=Mat4.translate(p.x+(sourceRoot and 0 or (motion.sway or 0)),0,p.z)
...
local function ballPose()
  ...
  local id=performanceId();local d=actionDuration("sendout") or 1.48
...
function T:sendoutStatus()
  ...
  local duration=actionDuration("sendout") or 1.48
...
function T:beginSendout(target,forward)
  ...
  return actionDuration("sendout") or 1.48
end
...
function T:finish(ctx,reason)
  battleKey=nil;age=0;actionKind=nil;actionAge=0;actionStrength=0;initialSendoutQueued=false;initialOpeningQueued=false;lastSendingOut=false;pendingFrustration=nil;pendingApproval=nil;pendingReaction=nil;resultSeen=nil;activeNow=false;activationReason=nil
end
