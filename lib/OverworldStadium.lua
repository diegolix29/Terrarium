...
function OverworldStadium.canRenderEntity(entity)
  ...
  -- Check StadiumPack for Gen1 (1-151) and Stadium2Pack for Gen2 (152-251)
  local ok1, available1 = pcall(StadiumPack.available, dex)
  if ok1 and available1 == true then return true, dex end
...
local function prepareOne(p, dex, dt)
  ...
  -- Use StadiumPack for Gen1 (1-151) and Stadium2Pack for Gen2 (152-251)
  if dex <= 151 then
    if type(StadiumPack.keep) == "function" then pcall(StadiumPack.keep, dex) end
  else
    local Stadium2Pack = V.require("Stadium2Pack")
    if type(Stadium2Pack.keep) == "function" then pcall(Stadium2Pack.keep, dex) end
  end
