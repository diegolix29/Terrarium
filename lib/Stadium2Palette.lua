...
function Stadium2Palette.recolour(rgba, normal, shiny)
...
function Stadium2Palette.decodeRare(data, offset)
...
function Stadium2Palette.applyRare(rgba, rare)
...
function Stadium2Palette.decodeRgba5551(bytes, pixelCount)
...
function Stadium2Palette.nativeTextureBytes(w, h, siz)
...
function Stadium2Palette.decodeNativeTexture(bytes, w, h, fmt, siz)
  if type(bytes) ~= "string" then return nil, "texture record is not bytes" end
  w = math.max(0, math.floor(tonumber(w) or 0))
  h = math.max(0, math.floor(tonumber(h) or 0))
  ...
  siz = math.floor(tonumber(siz) or -1)
  local pixels = w * h
  local expected = Stadium2Palette.nativeTextureBytes(w, h, siz)
  ...
  if fmt == 0 and siz == 2 then return Stadium2Palette.decodeRgba5551(bytes, pixels) end
...
function Stadium2Palette.fromTransformSource(source)
